...
function calculate(req, res, operation) {
    try {
        ...
        switch (operation) {
            ...
            case "modulo":
                if (num2 === 0) {
                    logger.error(`On ${operation} request, Modulo by zero`);
            ...
            case "sqrt":
                if (num1 < 0) {
                    logger.error(`On ${operation} request, Square root of negative number`);
                    return res.send(errorMessage("Cannot take square root of a negative number."));
                }
        ...
        const str = `${operation}` !== 'sqrt' ? `${num1} ${symbol} ${num2}` : `${symbol} ${num1}`;

        logger.log({ level: 'info', message: `Requested to ${operation.toUpperCase()}: ${str} = ${result}` });
        return res.send(`
            <div style="color: green; text-align: center;">
            <h2>Calculation Result</h2>
            <p style="color:black">${str} = <b>${result}</b></p>
            <br>
            <a href="/" style="text-decoration: none; background-color: #007BFF; color: white; padding: 10px 20px; border-radius: 5px; font-size: 16px;">Go Back</a>
            </div>
            `);
...
app.post("/power", (req, res) => calculate(req, res, "power"));
